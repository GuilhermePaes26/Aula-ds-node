...
app.post("/update/produto/:id_prod", async (req, res) => {
    const { id_prod } = req.params;
    const { nome_prod, estado_prod, descricao_prod } = req.body;
    try {
        const produto = await Produto.findOne({ where: { id_prod } });
        ...
        produto.nome_prod = nome_prod || produto.nome_prod;
        produto.estado_prod = estado_prod || produto.estado_prod;
        produto.descricao_prod = descricao_prod || produto.descricao_prod;
        await produto.save();
        res.redirect(`/produto/${produto.id_prod}`);
    } catch (error) {
        res.status(500).send(error.message);
    }
});
...
app.get("/doacao/find/:id_clie", async (req, res) => {
    try {
        ...
        // Transformar os dados das doações em objetos simples
        const doacoesPlain = doacoes.map(doacao => doacao.get({ plain: true }));

        // Renderiza a página de listagem passando os dados transformados
...
app.post("/doacao/novo", async (req, res) => {
    try {
        ...
        // Redireciona para a página de doações do cliente após criação
        res.redirect(`/doacao/find/${id_clie}`);
    } catch (error) {
        res.status(500).send(error.message);
    }
});

// Exibir os detalhes de uma doação
app.get("/doacao/detalhes/:id_doacao", async (req, res) => {
    try {
        ...
        res.render("doacao/detalhes", { doacao });  // Renderiza a página de detalhes
    } catch (error) {
        res.status(500).send(error.message);
    }
});

// Exibir formulário para editar a doação
app.get("/doacao/editar/:id_doacao", async (req, res) => {
    try {
        ...
        res.render("doacao/editar", { doacao, clientes, produtos });  // Renderiza o formulário de edição
    } catch (error) {
        res.status(500).send(error.message);
    }
});
...
app.post("/update/doacao/:id_doacao", async (req, res) => {
    try {
        ...
        res.redirect(`/doacao/detalhes/${doacao.id_doacao}`);  // Redireciona para os detalhes da doação
    } catch (error) {
        res.status(500).send(error.message);
    }
});
...
app.get("/delete/doacao/:id_doacao", async (req, res) => {
    try {
        ...
        res.render("doacao/confirmar_delete", { doacao });  
    } catch (error) {
        res.status(500).send(error.message);
    }
});
...
app.post("/delete/doacao/:id_doacao", async (req, res) => {
    try {
        const { id_doacao } = req.params;
        const doacao = await Doacao.destroy({ where: { id_doacao } });

        if (!doacao) {
            return res.status(404).send("Doação não encontrada.");
        }

        res.redirect(`/doacao/${req.body.id_clie}`);  
    } catch (error) {
        res.status(500).send(error.message);
    }
});
...
app.post("/alterar/reserva/:id_doacao", async (req, res) => {
    try {
        ...
        res.redirect(`/doacao/detalhes/${id_doacao}`);  
    } catch (error) {
        res.status(500).send(error.message);
    }
});
